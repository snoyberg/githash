...
-- should be the @.git@ directory itself.
--
-- @since 0.1.0.0
...
getGitInfo root = try $ do
  ...
  case mgit of
    Nothing -> throwIO GHEGitNotFound
    Just gitpath -> do
      ...
      let files3 = if packedExists then [packedRefs] else []
          ...
          run args = do
            let cp = (proc gitpath args)
                  { cwd = Just root
                  , std_err = NoStream
                  }
            eres <- try $ readCreateProcess cp ""
            case eres of
              Left e -> throwIO $ GHEGitRunFailed root args e
...
getGitRoot dir = do
    ...
    eres <- try $ readCreateProcess cp ""
    return $ case eres of
      Left e -> Left $ GHEGitRunFailed dir args e
      Right out -> Right $ takeWhile (/= '\n') out </> ".git"
...
data GitHashException
  ...
  | GHEGitRunFailed !FilePath ![String] !IOException
